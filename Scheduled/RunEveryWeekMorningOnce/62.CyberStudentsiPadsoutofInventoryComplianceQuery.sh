...
#  and get email addresses.  This version is for Students in the 
#  classroom, CYBER.
#####################################################################
...
DeviceReferenceFile="/tmp/Mosyle_active_iOS_Tagz_StudentiPads.txt"
DeviceGroup="23"
eMailDomain="@gatewayk12.net"
...
log_line() {
        ...
        echo "$TODAY =====>$LINE"
        #Log to file
        echo "62.CyberiPadsoutofInventoryComplianceQuery.sh ++> $TODAY =====> $LINE" >> $LOG
}
ParseIt() {
...
until [ -z "$UDID" ]; do	
	...
	if [ -z "$UDID" ]; then
		log_line "Last UDID has been processed."
		break
