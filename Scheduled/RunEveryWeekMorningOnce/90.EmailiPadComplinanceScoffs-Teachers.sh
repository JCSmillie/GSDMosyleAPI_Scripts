...
for emailme in `cat $TriggerFileofEmail`; do
	...
	echo "The iPad assigned to you ($email) with GSD tag #$AssetTag has not checked in for inventory since $LastSeen.  Per GSD Technology Proceedures (see the Incident IQ Knowledge base) all faculty iPads MUST CHECK IN at least every 21 days.  Please ensure at this time your device is charged and on a wifi network.  Try to use it.  If you no longer want the device please make arrangements through the Gator Help Desk to turn it back in.  A ticket has been created for this issue and a Gator Help Desk technican will be in touch to assist you with this matter.  Thank you." > /tmp/Email1.txt
